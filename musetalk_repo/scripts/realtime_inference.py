...
# The VAE decoder reconstructs the mouth from a 256x256 latent, so high-frequency
# detail (teeth, lip edges) comes out soft/blurry. An unsharp mask on the
# generated region restores some of that crispness. Strength is env-tunable
# (MUSETALK_SHARPEN, 0 disables; ~0.6 is a good default, >1.5 looks crunchy).
_SHARPEN_AMOUNT = float(os.environ.get("MUSETALK_SHARPEN", "0.6"))


def _unsharp(img, amount):
    """Lightly sharpen a BGR uint8 image via an unsharp mask."""
    if amount <= 0:
        return img
...
class Avatar:
    ...
    def process_frames(self, res_frame_queue, video_len, skip_save_images):
        ...
        while True:
            ...
            x1, y1, x2, y2 = bbox
            try:
                # The generated mouth region is 256x256; upscaling it back to the
                # face crop with the default INTER_LINEAR softens the lips. Use a
                # cubic kernel for noticeably crisper teeth/lip edges.
                res_frame = cv2.resize(res_frame.astype(np.uint8), (x2 - x1, y2 - y1),
                                       interpolation=cv2.INTER_CUBIC)
            except:
                continue
            # Restore high-frequency detail (teeth/lip edges) lost by the VAE decode.
            res_frame = _unsharp(res_frame, _SHARPEN_AMOUNT)
